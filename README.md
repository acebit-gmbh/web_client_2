...
- **Conditional access** -- shows the warning set on an entry in the Windows client before the entry is opened, before its second password is asked for, and before its URL is opened from the list: an informational warning as a notice, the others as a prompt that goes on only on *Continue*, with a checkbox to tick first where the entry asks for one (Server 20.0.0+)
...
- **Conditional access** -- the warning the listing carries for an entry (Server 20.0.0+) is answered before the client reads the entry at all: while a *confirm* or *verify* warning waits for an answer, no request for the entry is made - not even the second-password check, which is itself a read - and *Cancel* deselects the entry. A blocking warning that only the entry as read carries (the listing was older) keeps its content hidden until it is answered. The answer is held only by the open detail panel and never stored, so selecting the entry again asks again. The server returns the entry whether or not the warning was shown; showing it is up to the client, as it is in the Windows client.
...
- **Conditional-access warnings are read-only and shown before access** -- on Server 20.0.0+ every entry, compact and full alike, carries `warning`: `null`, or `{message, level, verify_text}` with `level` `info` (shown without blocking), `confirm` (goes on only on *Continue*) or `verify` (*Continue* only after ticking a checkbox labelled `verify_text`, or *I agree* when that is empty). A level the client does not know is treated as `verify`, and a missing key - an older server - shows nothing, the same as `null`. The warning is asked about before the full `GET` of the entry, which is what opening it is, and before its URL is opened from the list; the one-time code, the document download, copying and the detail's own links are reached only from the opened detail, so that one answer covers them - once per opening, where the Windows client asks at every access. The listing's copy decides before the read, and the entry as read is checked again: a listing can be minutes old, so a blocking warning that differs from the one answered keeps the content hidden until it is answered too. `message` is plain text, rendered as text with its line breaks. The key is set in the Windows client only: the entry form has no field for it and never sends it.
